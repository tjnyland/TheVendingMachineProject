...
namespace VendingMachineProject
{
    ...
    internal class Program
    {
        struct VendingMachine //Creates a structure called vending machine
        {
            ...
            public int stock;//Stock of the product
        ...
        static void Main(string[] args) //Main
        {
            ...
            VendingMachine[] table = new VendingMachine[20]; //Creates an array with all the values from the struct
                                                             ...
                                                             // NumberOfProducts = LoadProducts(ref table);
            for (int i = 0; i < 10; i++)
            {
                table[i].stock = 10; //stock of first 10 products manually added
        ...
        //LOADING PRODUCTS FROM TEXT FILES {EVEN THOUGH I GOT NO ERRORS THIS DIDN'T SEEM TO WORK, JUST DISPLAYED ALL VALUES AS NOTHING EVEN THOUGH THE TEXT FILE HAS VALUES IN IT}
        //HERE IS THE PAST CODE - decided just to add the items manually just so the program works as intended.

        //  static int LoadProducts(ref VendingMachine[] table)
        // {
        //  System.IO.StreamReader file = new System.IO.StreamReader("VendingMachineContent.txt");
        //   int ProductCount = 0;
        //  while (file.EndOfStream == false)
        // {
        //    VendingMachine Product;
        //   Product.name = file.ReadLine();
        //   Product.ID = int.Parse(file.ReadLine());
        //   Product.price = double.Parse(file.ReadLine());
        //   ProductCount += 1;
        // table[ProductCount] = Product;
        // }
        // file.Close();
        // return ProductCount;
        //  }
            ...
            try//Make sure the value entered is an int value!
            {
                ...
                if (ItemAmount > 0 && ItemAmount <= 10)
                {
                    for (int i = 0; i < ItemAmount; i++) //adds from index 10 as we already have items up to index 9, adds how many items user wants.
                    {
                        ...
                        Console.WriteLine("Please enter how much stock you have of this product"); //enter how much of the product you have
                        table[10 + i].stock = int.Parse(Console.ReadLine()); //stored.
                    }
                }
        ...
        static void ViewMachine(ref VendingMachine[] table, ref double bal)//passing table through it
        {
            ...
            {
                Console.WriteLine($"{table[i].ID} - {table[i].name} - £{table[i].price} - Stock: {table[i].stock}");// displays the vending machine items.
            }
                ...
                if (id <= 20) //if the ID is within the table[] array
                {
                    ...
                    if (bal >= table[id].price) // If the user has enough money
                    {
                        if (table[id].stock > 0)
                        {
                            ...
                            table[id].stock = table[id].stock - 1; //removes item from stock
                        }
                        else
                        {
                            Console.WriteLine("Invalid Stock | We have none of this item left.");
                        }
                    }
                    else
                    {
